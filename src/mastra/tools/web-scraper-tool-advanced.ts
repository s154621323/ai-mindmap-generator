...
export const webScraperAdvancedTool = createTool({
  id: 'scrape-webpage-advanced',
  description: '高级网页内容抓取工具，使用 Cheerio 解析，提取结构化信息',
  inputSchema: z.object({
    url: z.string().url().describe('要抓取的网页URL'),
    extractImages: z.boolean().optional().default(true).describe('是否提取图片信息'),
    extractLinks: z.boolean().optional().default(true).describe('是否提取链接信息'),
    maxContentLength: z.number().min(100).max(100000).optional().default(15000).describe('最大内容长度'),
    includeMetadata: z.boolean().optional().default(true).describe('是否提取元数据'),
    removeAds: z.boolean().optional().default(true).describe('是否移除广告内容'),
  }),
  outputSchema: z.object({
    ...
    })).optional().describe('链接列表'),
    images: z.array(z.object({
      url: z.string(),
      alt: z.string(),
      title: z.string().optional(),
    })).optional().describe('图片信息列表'),
    metadata: z.object({
...
async function scrapeWebPageAdvanced(
  ...
  removeAds: boolean
): Promise<WebPageContent & { url: string; scrapedAt: string; domain: string }> {
  ...
  if (removeAds) {
    ...
    $('[class*="ad"], [id*="ad"], [class*="advertisement"], [class*="banner"]').remove();
    $('[class*="popup"], [class*="modal"], [class*="overlay"]').remove();
  }

  // 解析内容
  const parsedContent = parseHTMLWithCheerio($, url, extractImages, extractLinks, maxContentLength, includeMetadata);
...
function parseHTMLWithCheerio(
  ...
  // 提取元数据
  const metadata = includeMetadata ? extractMetadataWithCheerio($) : { wordCount: content.length };
...
function extractMainContent($: cheerio.CheerioAPI, maxLength: number): string {
  ...
  // 移除不需要的元素
  contentElement.find('script, style, noscript, nav, header, footer, aside, .sidebar, .menu, .navigation').remove();
...
// 提取标题结构
function extractHeadingsWithCheerio($: cheerio.CheerioAPI): Array<{ level: number; text: string; id?: string }> {
...
// 提取链接
function extractLinksWithCheerio($: cheerio.CheerioAPI, baseUrl: string): Array<{ url: string; text: string; isExternal: boolean }> {
...
// 提取图片
function extractImagesWithCheerio($: cheerio.CheerioAPI, baseUrl: string): Array<{ url: string; alt: string; title?: string }> {
...
// 智能摘要生成
function generateSmartSummary(content: string, headings: Array<{ level: number; text: string }>): string {
  // 如果有标题，优先使用标题信息
  if (headings.length > 0) {
    const mainHeadings = headings.filter(h => h.level <= 2).slice(0, 3);
    if (mainHeadings.length > 0) {
      return `主要内容包括：${mainHeadings.map(h => h.text).join('、')}。${content.substring(0, 150)}...`;
    }
  }
